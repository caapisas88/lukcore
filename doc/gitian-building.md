...
*Setup instructions for a Gitian build of Luk Core using a Debian VM or physical system.*
...
- [Building Luk Core](#building-luk-core)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-luk-host/README.md).
...
Clone the git repositories for luk and Gitian.
...
in the luk repository under 'Fetch and create inputs' to install sources which require
...
To build Luk Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the luk repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/luk/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/luk/luk
...
For example:
```bash
URL=https://github.com/laanwj/luk.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit luk=${COMMIT} --url luk=${URL} ../luk/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit luk=${COMMIT} --url luk=${URL} ../luk/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit luk=${COMMIT} --url luk=${URL} ../luk/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the luk git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../luk/contrib/gitian-descriptors/*|sort|uniq )
...
LUKPATH=/some/root/path/luk
SIGPATH=/some/root/path/luk-detached-sigs

./bin/gbuild --url luk=${LUKPATH},signature=${SIGPATH} ../luk/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/luk-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/luk-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/luk-osx-build.assert
```
...
[luk-core/gitian.sigs](https://github.com/luk-core/gitian.sigs/) repository, or if that's not possible create a pull
