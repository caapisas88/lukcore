...
###   lukcore.org and luk.org and compares them.
...
WORKINGDIR="/tmp/luk_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://lukcore.org"
HOST2="https://luk.org"
BASEDIR="/bin/"
VERSIONPREFIX="luk-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'luk-',
...
if [ $? -ne 0 ]; then
   echo "luk.org failed to provide signature file, but lukcore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "luk.org and lukcore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Luk Core binary release signing key installed?"
   fi
