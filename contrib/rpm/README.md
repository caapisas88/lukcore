...
The RPM spec file provided here is for Luk-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://luk.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Luk git repository](https://github.com/luk/luk) but are not part of
...
Currently only `Source10` is used, for the example `luk.conf` file.
...
in the luk git repository. At present only an SVG version of the Luk
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the luk
...
needed when 0.12.1 is released, a proper fix is already in the Luk git
master branch.
...
idea to build Luk Core against a static version of that library compiled
...
The `luk.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `luk-server` package. At this point, most RPM based
...
The `luk-server` package should have SELinux support. How to properly do
...
Luk-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/luk/). He has been
packaging Luk for Fedora at least since 2012.
...
4. I renamed the `luk` package that contains the Qt GUI to `luk-core` as
that appears to be how the general population refers to it, in contrast to
`luk-xt` or `luk-classic`. I wanted to make sure the general population
